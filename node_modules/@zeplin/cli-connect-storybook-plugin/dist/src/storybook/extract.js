...
function specFromStory({ id, kind, name, parameters: { component = {}, framework, fileName, docs } = {} }, componentPathMap = {}) {
...
const extract = global => {
    const { __STORYBOOK_CLIENT_API__, STORYBOOK_ENV, STORYBOOK_REACT_CLASSES } = global;
    if (!__STORYBOOK_CLIENT_API__) {
        throw new Error(`Zeplin CLI requires Storybook version at least 3.4. Please update your Storybook!`);
    }
    const componentPathMap = {};
    if (STORYBOOK_ENV === "react" && STORYBOOK_REACT_CLASSES) {
        Object.values(STORYBOOK_REACT_CLASSES).forEach(src => componentPathMap[src.docgenInfo.displayName] = src.path);
    }
    ...
    return __STORYBOOK_CLIENT_API__
        .getStorybook()
        .map(({ kind, stories }) => stories.map(({ name }) => specFromStory({
        kind,
        name,
        parameters: storyStore.getStoryAndParameters &&
